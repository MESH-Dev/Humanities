jQuery( document ).ready( function( $ ) {
	...
	function change_date( dateString, element ) {
		var parts = dateString.split('-');
		var date = new Date(parseInt(parts[0]), parseInt(parts[1]) - 1, parseInt(parts[2]));
		var event = $.Event('changeDate', { date: date, dates: [date] });
		element.bootstrapDatepicker('update', date);
		element.trigger('changeDate', event);
	}
			...
			$.post(
				...
				function( response ) {
					...
					if ( response.success ) {
						...
						change_date($('.tribe-mini-calendar-wrapper').find('.tribe-mini-calendar').attr('data-eventdate'), $('#tribe-bar-date'));
					}
				}
			);
		} )
			...
			$.post(
				...
				function( response ) {
					if ( response.success ) {
						...
						change_date($this.attr('data-day'), $('#tribe-bar-date'));
					}
				}
			);
		} );
