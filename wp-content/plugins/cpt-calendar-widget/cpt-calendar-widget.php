...
function ucc_get_calendar( $post_types = '' , $initial = true , $echo = true ) {
  ...
  $currentmonth = $wp_locale->get_month( $thismonth );
  $currentyear = date( 'Y' , $unixmonth );
  ...
  $nextmonthyear = $next_month = date('Y',strtotime( $currentmonth . ' next month'));
  $calendar_output = '<table id="wp-calendar" summary="' . esc_attr__( 'Calendar' ) . '">
  <caption><a href="?month='.$previousmonth.'">' . $previousmonth . ' '. $previousmonthyear .'</a></caption>
  <caption><a href="?month='.$currentmonth.'">' . $currentmonth . ' '. $currentyear .'</a></caption>
  <caption><a href="?month='.$nextmonth.'">' . $nextmonth . ' '. $nextmonthyear .'</a></caption>
  <thead>
  <tr>';
  ...
  $dayswithposts = $wpdb->get_results( "SELECT DISTINCT DAYOFMONTH( {$wpdb->postmeta}.meta_value )
    ...
    AND ( {$wpdb->postmeta}.meta_key = 'DateStart' AND ( YEAR ( {$wpdb->postmeta}.meta_value ) = '{$thisyear}' AND MONTH ( {$wpdb->postmeta}.meta_value ) = '{$thismonthfix}' ) );", ARRAY_N);
  ...
  for ( $day = 1 ; $day <= $daysinmonth ; ++$day ) {
    ...
    if ( in_array( $day , $daywithpost ) ) // any posts today?
        //$calendar_output .= '<a class="winner" href="' . get_day_link( $thisyear , $thismonth , $day ) . "\" title=\"" . esc_attr( $ak_titles_for_day[$day] ) . "\">$day</a>";
        $calendar_output .= '<a class="winner" href="' . site_url('/whats-happening/?date=' . $thisyear . '-' . $thismonth . '-' . $day ) . "\" title=\"" . esc_attr( $ak_titles_for_day[$day] ) . "\">$day</a>";
    else
      $calendar_output .= $day;
    $calendar_output .= '</td>';
